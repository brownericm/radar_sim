...
from scipy import interpolate, sqrt, special
...
def refl_coef(psig, eps_p, eps_pp):
    """
    Generate the reflection coefficient (vertical and horizontal for a particular surface.
    Inputs:
...
def atmo_absorp(ht, hr, freq, beta):
    """
    atmo_absorb calculates the atmospheric absorbtion and attenuation due to O2 and H2O between radar and target
    Inputs:
    ...
    # hr = hr * 1000
    h_vec = linspace(hr, ht,
                     999)  # Make linspace() or arange() array of heights so we can integrate over an array of ranges?
